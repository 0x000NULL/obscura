...
fn test_delegation_marketplace_functionality() {
    ...
    assert!(
        retrieved.is_some(),
        "Should be able to retrieve a created listing"
    );
    assert_eq!(
        retrieved.unwrap().id,
        "listing1",
        "Listing IDs should match"
    );
    ...
    assert!(
        result.is_err(),
        "Creating an offer for a non-existent listing should fail"
    );
    ...
    assert!(
        result.is_err(),
        "Creating a transaction with invalid offer should fail"
    );
}
...
fn test_validator_reputation_manager() {
    ...
    assert!(
        reputation.total_score > 0.9,
        "Reputation score should be updated to the assessment value"
    );
    ...
    assert_eq!(
        updated_reputation.update_count, 2,
        "Update count should be 2"
    );
    assert!(
        updated_reputation.total_score < 0.95 && updated_reputation.total_score > 0.85,
...
fn test_stake_compounding_manager() {
    ...
    assert!(
        result.is_ok(),
        "Starting a compounding operation should succeed"
    );
    ...
    assert!(
        result.is_err(),
        "Starting an operation with duplicate ID should fail"
    );
    ...
    assert!(
        result.is_ok(),
        "Updating status for a valid operation should succeed"
    );
    ...
    assert!(
        result.is_err(),
        "Updating status for invalid operation should fail"
    );
}
...
fn test_validator_diversity_manager() {
    ...
    assert!(
        retrieved.is_some(),
        "Should be able to retrieve validator geo info"
    );
    assert_eq!(
        retrieved.unwrap().country_code,
        "US",
    ...
    assert_eq!(
        report.validator_count, 2,
        "Report should include correct validator count"
    );
    assert_eq!(
        report.entity_count, 1,
        "Report should include correct entity count"
    );
    assert!(
        report.metrics.geographic_diversity > 0.0,
        "Report should include diversity metrics"
    );
}
...
fn test_hardware_security_manager() {
    ...
    assert!(
        result.is_ok(),
        "Adding validator with sufficient security should succeed"
    );
    ...
    assert!(
        result.is_err(),
        "Adding validator with insufficient security should fail"
    );

    // Test security level verification
    assert!(
        security_manager.verify_security_level(&validator_id1),
        "Validator1 should pass security verification"
    );
    assert!(
        !security_manager.verify_security_level(&validator_id2),
        "Validator2 should fail security verification"
    );
...
fn test_contract_verification_manager() {
    ...
    assert!(
        verification_manager.is_contract_verified("contract1"),
        "Contract should be verified"
    );
    assert!(
        !verification_manager.is_contract_verified("nonexistent"),
        "Non-existent contract should not be verified"
    );
    ...
    assert!(
        !verification_manager.is_contract_verified("contract2"),
        "Unverified contract should report as not verified"
    );
}
...
fn test_proof_of_stake_integration() {
    ...
    pos.reputation_manager
        .update_reputation(validator1_hex.clone(), assessment1);
    ...
    pos.security_manager
        .add_security_info(validator1_hex.clone(), security_info1)
        .unwrap();
    ...
    pos.diversity_manager
        .add_validator_geo(validator1_hex.clone(), geo_info1);
    ...
    pos.staking_contract
        .validators
        .insert(validator1_id.clone(), validator1_info);
    ...
    pos.diversity_manager
        .add_validator_geo(validator2_hex.clone(), geo_info);
    ...
    pos.security_manager
        .add_security_info(validator2_hex.clone(), security_info2)
        .unwrap();
    ...
    pos.reputation_manager
        .update_reputation(validator2_hex.clone(), assessment2);
    ...
    assert!(
        result.is_err(),
        "Validator with low reputation should be rejected"
    );
...
fn test_security_error_conditions() {
    ...
    pos.diversity_manager
        .add_validator_geo(validator3_hex.clone(), geo_info3);
    ...
    pos.staking_contract
        .validators
        .insert(validator3_id.clone(), validator3_info);

    // Update all enhancements first to ensure diversity metrics are calculated
    pos.update_enhancements(current_time)
        .expect("Updating enhancements should succeed");

    // Now test validation for validator2 which has good reputation but no security info
    let result = pos.validate_new_validator(&validator2_id);
    assert!(
        result.is_err(),
        "Validator with no security attestation should fail validation"
    );
    let error_msg = result.unwrap_err();
    assert!(
        error_msg.contains("attestation") || error_msg.contains("security"),
        "Error message should mention security attestation, got: {}",
        error_msg
    );
    ...
    assert!(
        result1.is_ok(),
        "Validator with good reputation and security level should pass validation"
    );
}
