...
fn test_key_encryption_wrong_password() {
    ...
    // Attempting to decrypt with wrong password should return None
...
fn test_key_encryption_data_integrity() {
    ...
    let mut encrypted = encrypt_keypair(&keypair_tuple, password);
...
fn test_key_encryption_format() {
    ...
    assert!(decrypt_keypair(&short_data, password).is_none());
    
    // Test 2: Only salt
    let salt_only = encrypted[0..16].to_vec();
    assert!(decrypt_keypair(&salt_only, password).is_none());
    
    // Test 3: Salt and nonce (no ciphertext)
    let salt_and_nonce = encrypted[0..28].to_vec();
    assert!(decrypt_keypair(&salt_and_nonce, password).is_none());
}
