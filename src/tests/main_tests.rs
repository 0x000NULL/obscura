...
mod main_tests {
    ...
    use crate::{init_crypto, init_wallet, init_blockchain, init_consensus, init_networking, 
               start_network_services, process_mempool};
    ...
    fn test_process_mempool() {
        ...
        assert_eq!(processed, 0, "Should process 0 transactions in empty mempool");
    ...
    async fn test_full_node_initialization() {
        ...
        fn start_test_network_services(_mempool: Arc<Mutex<Mempool>>) -> std::thread::JoinHandle<()> {
        ...
        network_handle.join().expect("Network thread should complete without panicking");
        
        assert!(true, "Full node initialization and limited run should succeed");
    }
